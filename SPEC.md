...
- H2 `Tag(element)` from `frontage.head` puts one whole element in the page's `<head>` — a `<link rel=canonical>`, an hreflang set, a favicon, a JSON-LD graph, an inline script that must run before first paint — and renders nothing where it stands. Off the browser it records the element's HTML and `frontage prerender`/`frontage site` splice it into `<head>`, once however many mounts asked for it; in the browser it is a mount into `document.head` that goes away with its owner. [0.13.6]
...
- L5 `frontage.i18n.hreflang(site, path)` writes the `<link rel="alternate" hreflang=…>` tags **into the page's `<head>`** (through `Tag`, so it renders nothing where the layout puts it), `x-default` included, absolute when `BASE` is set; `switcher(site, path, labels=…)` is the language switcher **as plain links**, with the current language a `<span aria-current>` rather than a link to itself. [0.13.1, into the head 0.13.7]
...
- S11 A page of a site is rendered **without hydration markers** — no `data-fr-h`, no `<!--h-->`, no fences — because it will never hydrate. An island does, and gets its own pass with them on. What an author wrote stays: `comment(text)` renders an HTML comment, and only the framework's own markers are dropped. [0.13.6]
- S12 `public/` and `STATIC` copy dot directories: `.well-known/` is where `tdmrep.json`, `security.txt` and an app-association file live, and a blanket dotfile rule drops the one dot directory the web standardised. `.DS_Store`, `.git`, `.gitignore`, `__pycache__` and `*.pyc` are named instead. [0.13.6]
...
- V2 `comment(text)` renders an HTML comment, for the rare one that is functional — Cloudflare's `<!--email_off-->`, a host's marker. A string is escaped and a template's `<!-- -->` is a hydration marker, so there is no other way to write one; `to_html` keeps it and drops only the framework's own markers. [0.13.6]
