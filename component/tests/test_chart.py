...
`frontage_chart.plot` imports `chart`, which in a page is the module `frontage build`
...
def js(monkeypatch):
    ...
    module.draw = fake.draw
    module.destroy = fake.destroy
    monkeypatch.setitem(sys.modules, "chart", module)
    for name in [n for n in sys.modules if n.startswith("frontage_chart")]:
...
def test_each_kind_says_which_it_is(js, kind):
    import frontage_chart
    from frontage import render_to_string

    chart = getattr(frontage_chart, f"{kind}_chart")
