...
`frontage_map.map` imports `map`, which in a page is the module `frontage build` registered
...
def js(monkeypatch):
    ...
    for name in [n for n in sys.modules if n.startswith("frontage_map")]:
...
def test_a_map_is_an_element_with_a_class_and_a_height(js):
    ...
    from frontage_map import map_view
