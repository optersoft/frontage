...
| [`frontage.remote`](frontage-polars/) | 0.6 KB of JavaScript | **polars on the server, small answers in the page**: named queries behind a FastAPI router, a `Resource` per query in the browser, windows for the grid, and a Server-Sent Events stream so a change on the server reaches every page. The one package here with a server half, and it is opt-in (`[server]`) |
...
| [`frontage.chat`](frontage-chat/) | 1.0 KB gzipped | **a chat that streams**: the log, the input and a rating, with each message's text in a `Signal` — a token appends to one text node and the bubbles above it are never rebuilt. A server half (`[chat]`) holds the API key and answers Server-Sent Events |
...
```sh
pip install frontage frontage-chart frontage-table frontage-map frontage-schema frontage-supabase
```

That is the whole install — `frontage.remote` is left out on purpose, being the one with a
server half; `pip install frontage[polars]` where that server runs. `frontage build`
discovers each package by its entry point, copies its browser assets, links its stylesheet, and
packs its Python — so `from frontage.chart import line_chart` resolves inside the page with
...
`frontage.chart/plot.py` is the pattern in twenty lines: a `NodeRef` for the element the
...
Apache 2.0. uPlot is vendored under `frontage-chart/frontage.chart/_browser/` (MIT).
