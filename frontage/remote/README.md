...
```sh
pip install frontage            # the browser half, packed into the page by `frontage build`
pip install "frontage[polars]"  # the server half: polars, FastAPI, uvicorn
```
...
```python
from frontage.remote import remote
from frontage.chart import bar_chart
from frontage.table import table
