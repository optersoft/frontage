...
uPlot is vendored (MIT, see `frontage.chart/_browser/uplot.js`), so a build needs no network
...
`frontage.remote`' `series` hands back — and draws those as they are, with no copy.
