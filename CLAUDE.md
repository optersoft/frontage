...
| `tools/gallery.py` | `mk gallery`: builds every app in its `APPS` list with the real `frontage build`, loads each cold in Chromium, and writes `www/gallery/` with the measured size and start time on every card. A broken app fails the build; a slower one changes the number on the page. `site.build` runs it and then preserves the result rather than rebuilding it. The page is styled with Tailwind: `tools/gallery.tailwind.css` is the input (`source(none)` plus one `@source` at `gallery.py`, since the template is the only place a class name appears) and **`tools/gallery.css` is the compiled output, committed** — the deploy builder copies it rather than downloading Tailwind's 100 MB CLI. Its first line stamps the digest of the template it was built from, so a template edit rebuilds it (`mk gallery --css` forces one), and a build with no Tailwind ships the committed file with a warning instead of an unstyled page |
