...
| `tools/site_css.py` | `mk site.css`: the site's stylesheet, Tailwind compiled by frontage's own `tailwind` command. Input `web/site.tailwind.css` (`source(none)` plus three `@source` lines — the two static pages and the gallery's template, the only places a class name appears), output the **committed** `web/site.css`, whose first line stamps the digest of everything it was built from. So the normal path is a copy, an edit to any source rebuilds it, and a host that cannot run Tailwind (the deploy builder, where the CLI is a 100 MB download) ships the committed file with a warning rather than an unstyled page |
