...
**0.13.0** — islands and static pages (`ISLAND.md` steps A–C), content collections (D) and
`frontage site` (E) — and past milestone **M11** (0.9.0: the WebAssembly boot, the framework as precompiled bytecode, a dev
...
| `ISLAND.md` | the 0.11 plan (2026-09-09), **steps A–C shipped in 0.11, D in 0.12.0, E in 0.13.0**: what Astro does for a content site and how frontage does it — a page with nothing interactive ships no runtime, an island is a `mount` with a trigger (`when="visible"`), content is a collection checked by `frontage.schema`, pages are files. Measured on the two company Astro sites it has to replace; the prerendered counter is 455 bytes waiting for 265 KB, which is the whole case |
