...
| `ISLAND.md` | the 0.11 plan (2026-09-09), **A–C in 0.11, D in 0.12.0, E in 0.13.0, F in 0.13.1, G in 0.13.3 — `web/` is on it; `site/` is what is left**: what Astro does for a content site and how frontage does it — a page with nothing interactive ships no runtime, an island is a `mount` with a trigger (`when="visible"`), content is a collection checked by `frontage.schema`, pages are files. Measured on the two company Astro sites it has to replace; the prerendered counter is 455 bytes waiting for 265 KB, which is the whole case |
| `API.md` | the HTTP server (2026-09-10): FastAPI's shape on **axum**, with the handlers on this repository's own runtime — `rust/api/`, one `Vm` per worker thread, no CPython and no GIL. §6.1 is built and its gate is met: **5.97× Granian + FastAPI** on a body-echo route, and reading that body costs us 5% where it costs Granian 48%, which is the one-crossing rule measured. Its point is §4.6 — the same `frontage.schema` record validates a form in the page and the body that form posts. Was a sibling repository until it was merged here; §3 says why either way |
...
| `frontage/i18n.py` | what a page in more than one language says about itself (0.13.1): `hreflang(site, path)` (the alternates, `x-default` included, **through `head.Tag`** — it renders nothing where the layout writes it and the tags land in `<head>`, which is the only place a crawler reads them; 0.13.6) and `switcher(site, path, labels=)` — the language switcher as **plain links**, because the build made every page and `Site.translate` is an answer rather than a guess. Locale *routing* is in `cli/site.py`: `pages/[lang]/…`, `LOCALES` in `site.py` with the first entry default and no segment of its own |
...
| `examples/` | one page per example, the browser suite's and the benchmark's material (`tracker/` is the whole framework in one app: routes, a store kept by `reconcile`, memo-loaded details, a transactional toggle with `Optimistic`, a Portal modal, an `ActionForm`, boundaries; prerendered and hydrated in the tests too; `lazy/` is one route in a chunk of its own, the only example a dev server cannot show — a chunk exists only in a build; `wasm/` calls a 41-byte hand-assembled WebAssembly library through `data-fr-js`, which the docs quote byte for byte; `rustlib/` is the same idea in real Rust and the template for doing it yourself (a `no_std` cdylib, 1 KB, thirty lines of glue) — its page measures why such a library takes the whole list once instead of taking it per call; `chart/` does the same on a real one, uPlot, and is the evidence behind `COMPONENTS.md`; `api/` is the <page:python/frontage/api> chapter's app and the smallest complete answer to
"my page needs a server": one `record` in `contract.py`, a `Form` over it in the page and the
same object validating the body in `server.py`, plus an SSE seat count — ⚠ the contract is its
own module because importing `server.py` from the page makes `frontage build` ship it to the
browser *without* `frontage_api`, silently, and the page then fails at boot; `weather/` is
Streamlit's Seattle Weather demo ported line for line — the 1,461-row dataset ships in `data.py`, two charts are uPlot and three are divs, and it is the app to point at when someone asks what frontage is *for*). Each page is one boot tag; the dev server answers `<dir>/_frontage/…` per directory and builds both archives from disk, so an edit to an example or to the framework shows on reload with nothing to rebuild. Not published: the academy chapters run their own apps in the page (the released wheel, through `frontage build` in each chapter's pipeline), so a chapter's code block is both what the reader reads and what runs |
| `web/` | **frontage.optersoft.com, built by frontage** (0.13.3, `ISLAND.md` step G): a `frontage site` whose chrome is **`optersoft_brand`** — a path dependency on the sibling checkout, `mk site.build` clones it when absent. `pages/` is the landing page, `pages/gallery/index.py` (the numbers from `web/data/gallery.json`, which `tools/gallery.py` writes) and `pages/404.py`, whose `PATH = "/404.html"` makes it the file a host serves for an unmatched path; `layouts/site.py` is the chrome with this site's brand and links; `tailwind.css` imports the chrome's `chrome.css` and `--tailwind` compiles one stylesheet; `public/` holds `_headers`, `_redirects`, the playground and `runner.html`. **Every page ships no runtime** — four requests and no `<script src>`. `mk site.dev` serves it with a live rebuild (`frontage serve --prerender`); `mk site.build` writes it straight into `www/` beside the built gallery apps, the runtime twice (once under the playground, once at the root for the runner) and every released wheel. Astro, `@optersoft/astro`, `node_modules` and `tools/site_css.py` are gone with it |
...
| `tools/gallery.py` | `mk gallery`: builds every app in its `APPS` list with the real `frontage build`, loads each cold in Chromium, and writes `www/gallery/` with the measured size and start time on every card. A broken app fails the build; a slower one changes the number on the page. `site.build` runs it and then preserves the result rather than rebuilding it. The page that renders them is `web/pages/gallery/index.py` |
| `typings/` | ty stubs for the browser-only modules |
| `rust/api/` | **the HTTP server** (`API.md`): axum over the VM, one interpreter per worker thread, one OS thread per worker so a suspended coroutine resumes on the `Vm` it started on. `_host` is the hook a Rust event settles a Python future through, and a coroutine is driven in Rust rather than by an asyncio `Task`, so an `async def` that never suspends never reaches the event loop. ⚠ A workspace member but **not a default member** — the runtime's gate must not compile tokio to check a bytecode change — and `[profile.api]` exists for `panic = "unwind"`. The routes describe themselves: `/openapi.json` is the document and `/docs` a dependency-free page over it (`frontage_api/openapi.py`, `docs.py`; `App(docs=None, openapi_url=None)` gives the paths back) — a route's prose is its handler's docstring, which the server keeps and `fpy --compile` drops, so a page pays nothing for it. `tests/routes.py` is 13 assertions over the real binary, `--stress` runs them collecting at every safe point, `bench/` is the measurement. `src/http.rs` is **`_http`**, what a handler talks to
anything through: `request(...)` answers an asyncio future a tokio task settles, and a
...
- **The academy chapters are part of a release.** The docs live in
  ...
  **Each chapter's app is a repository** at `gitlab.com/optersoft/python/frontage-<chapter>`
  (checkout `~/gitlab/python-frontage-<chapter>`), exported to GitLab Pages by its pipeline with
