...
def test_bounds_and_multiple_of():
    ...
    assert errors(integer(gt=0, lt=10, multiple_of=3), 10) == [("$", "must be less than 10"), ("$", "must be a multiple of 3")]
...
@pytest.mark.parametrize("value", ["a@@b.c", "a@b", "@b.co", "a@", "a b@c.d", ".a@b.co", "a@-b.co", "a@b..co", "üser@b.co"])
def test_emails_that_are_not(value):
    assert not email().is_valid(value)


@pytest.mark.parametrize("value", ["http://localhost:8000", "https://example.com/p?q=1#f", "ftp://x.y/z", "http://127.0.0.1"])
...
@pytest.mark.parametrize(
    "value",
    ["123e4567-e89b-12d3-a456-42661417400", "123e4567e89b12d3a456426614174000", "123e4567-e89b-12d3-a456-42661417400g", "0x3e4567-e89b-12d3-a456-426614174000", " 23e4567-e89b-12d3-a456-426614174000"],
)
...
@pytest.mark.parametrize("value", ["2026-02-30", "2023-02-29", "1900-02-29", "2026-13-01", "2026-9-7", "2026-09-07T00:00", "²026-09-07"])
...
@pytest.mark.parametrize(
    "value",
    ["2026-09-07T21:17", "2026-09-07T21:17:05", "2026-09-07T21:17:05.123Z", "2026-09-07 21:17:05+02:00", "2026-09-07T21:17:05-0530", "2026-09-07T23:59:60Z"],
)
def test_datetimes_that_are(value):
    assert iso_datetime().is_valid(value)


@pytest.mark.parametrize("value", ["2026-09-07", "2026-09-07T24:00", "2026-09-07T21:60", "2026-09-07T21:17:05+25:00", "2026-09-07X21:17", "2026-09-07T21:17:05."])
...
def test_a_schema_prints_as_the_code_that_builds_it():
    ...
    assert source == (
        ...
        "('age', integer(gt=0, le=150), None), ('tags', array(text()), <class 'list'>))"
    ) or "('tags', array(text()), " in source
    assert repr(one_of(optional(uuid()), pattern("^x", strip=True))) == "one_of(optional(uuid()), pattern('^x', strip=True))"
...
def test_repr_round_trips_through_eval():
    ...
    Order = record(("user", record(("name", text(min=1)), ("n", optional(integer()), None))), ("kind", literal("a", "b")))
    again = eval(repr(Order), vars(fs))
    assert repr(again) == repr(Order)
    assert again.validate({"user": {"name": ""}, "kind": "c"})[1] == Order.validate({"user": {"name": ""}, "kind": "c"})[1]
