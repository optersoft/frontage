...
from frontage.schema import array, email, integer, literal, optional, record, text
from frontage.schema.jsonschema import from_json_schema, to_json_schema
...
PYDANTIC = {
    "$defs": {
        "Address": {
            "properties": {
                ...
                "zip": {"title": "Zip", "type": "string", "minLength": 5},
            },
            "required": ["city", "zip"],
            "title": "Address",
            "type": "object",
    ...
    "properties": {
        ...
        "age": {
            "anyOf": [{"exclusiveMinimum": 0, "maximum": 150, "type": "integer"}, {"type": "null"}],
            "default": None,
            "title": "Age",
        },
...
def test_the_read_schema_validates_and_fills_defaults():
    ...
    assert User.validate({"id": 1, "name": "A", "email": "a@b.co", "address": {"city": "X", "zip": "1"}})[1] == [
        ("$.address.zip", "at least 5 characters")
    ]
...
def test_to_json_schema_is_what_from_json_schema_reads():
    User = record(
        ("id", integer(ge=0)),
        ("email", email()),
        ("age", optional(integer(gt=0)), None),
        ("kind", literal("a", "b"), "a"),
        ("tags", array(text(max=3))),
    )
...
def test_a_non_empty_list_default_is_fresh_and_prints_as_source():
    Doc = from_json_schema(
        {"type": "object", "properties": {"tags": {"type": "array", "items": {"type": "string"}, "default": ["new"]}}}
    )
    assert repr(Doc) == "record(('tags', array(text()), fresh(['new'])))"
    assert Doc.parse({})["tags"] is not Doc.parse({})["tags"]
    assert to_json_schema(Doc)["properties"]["tags"]["default"] == ["new"]
    assert to_json_schema(from_json_schema(PYDANTIC))["properties"]["tags"]["default"] == []


def test_a_models_own_defs_resolve_wherever_the_document_put_the_model():
    # FastAPI hoists a model's $defs to components/schemas; a document that embeds
    # `model_json_schema()` as is leaves them under the model, and the $ref still says #/$defs.
    document = {"components": {"schemas": {"User": PYDANTIC}}}
    User = from_json_schema({"$ref": "#/components/schemas/User"}, document)
    assert "('address', record(('city', text()), ('zip', text(min=5))))" in repr(User)
