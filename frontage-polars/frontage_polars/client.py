...
`frontage_table.table`: the grid asks for the window it shows, sorted and searched on the
server, and the rest of the frame stays there.
...
class Frame:
    ...
    def series(self, *names):
        """`[x, y1, …]`, what `frontage_chart` draws: the named columns, in that order."""
...
async def _get_browser(url):
    import remote as _js  # the JavaScript half, registered by the build; absent on CPython
...
class Remote:
    def __init__(self, base, events=True, fetch=None):
    ...
    def rows(self, name, **params):
        """A windowed source over the named query, for `frontage_table.table`."""
        self._listen()
    ...
    async def _json(self, url):
        ...
        if status != 200:
            try:
                message = json.loads(text).get("detail", text)
            except (ValueError, AttributeError):
                message = text
            raise RemoteError(status, message)
...
def remote(base, events=True, fetch=None):
    """`Remote(base)`: the client for a `Sources` router mounted at `base`."""
    return Remote(base, events=events, fetch=fetch)
